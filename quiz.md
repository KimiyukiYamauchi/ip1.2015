...
1. LANの接続装置で、ゲートウェア/ルータ/ハブの違いを説明して下さい  
ゲートウェイ => 異なるネットワークを相互接続  
ルータ => 送られてきたパケット　(データ)をあて先のネットワークまで中継(ルーティング)  
する．不要な通信を廃棄するフィルタリング機能をもつ  
ハブ(HUB) => スター型LANで使われる集線装置．各機器はハブに接続され、ハブを通して  
他の機器と通信する
1. 技術的脅威である「キーロガー」とはどのようなものですか説明して下さい  
キーボードからの入力を監視、記録するソフトウェア．利用者のパスワードを盗むためなどに  
利用される
1. ペネトレーションテストとは何ですか説明して下さい  
実際にコンピュータやネットワークへの攻撃や侵入を試みることで、セキュリティ上の課題を発見する  
テスト手法です．特に、インターネット接続のシステムにおいては、外部からの攻撃に対して安全かどうか  
を検証するために有効
...
ペンタブレットなどで描く  
1. リレーショナル・データベースの表における主キー(プライマリキー)の  
目的を説明して下さい  
...
<p>
	...
	=> 4
1. プラグアンドプレイとは何ですか説明して下さい  
=> 周辺機器を追加した際、OSが周辺機器を検出して自動的に設定を行う仕組み  
1. 信頼性設計における「フェールセーフ」とは何ですか説明して下さい  
=> システムの一部に障害が発生した場合、その影響をできるだけ安全な方向へ傾ける  
考え方  
1. 「RAID0」と「RAID1」の違いを説明して下さい  
RAID0 => ストライピング  
RAID1 => ミラーリング  

## 11/12

1. プロジェクトとは何ですか．説明して下さい  
ある目標のために期間や予算を決めてチームを構成し、活動すること  
1. プロジェクトメンバー７人で一対一でコミュニケーションする場合、伝達経路は  
幾つになりますか  
7つから2つを取り出す組み合わせなので、7×6÷2 => 21  
1. 「サービスデスク」とは何ですか．説明して下さい  
利用者専用の窓口を設けて、ユーザからの問合せを受け付ける組織(機能)  
1. 「ファシリティマネジメント」とは何ですか．説明して下さい  
設備や環境面で行うセキュリティ管理  
1. 「ITガバナンス」とは何ですか．説明して下さい  
経営戦略とIT戦略との整合性や投資効果、組織の在り方などの強化するための  
体制づくりや意思決定のしくみ  

## 11/11

1. システムの開発プロセスにおけて「企画プロセス」ではどのようなことを行いますか  
説明して下さい  
経営戦略・システム戦略に則り、システム化計画の策定を行う  
1. システムの開発プロセスにおいてSIベンダに出す「RFP」とは何ですか  
説明して下さい  
提案依頼書(Request For Proposal)、必要とするITサービスの調達先を  
選定するために、その発注候補先に対して、どんなサービスを調達したいかを明示するもの  
1. 「ホワイトボックステスト」と「ブラックボックステスト」の違いを説明して下さい  
ホワイトボックステスト => 内部構造を意識してテスト項目を洗い出す  
ブラックボクステスト => ユーザインタフェースに着目してテスト項目を洗い出す  
1. ソフトウェア開発モデルの「プロトタイピングモデル」について説明して下さい  
プロトタイプ(試作品)をはじめに作成し、ユーザの要求を確認する．その内容をもとに  
実際の開発を行っていく手法  
1. 「リバースエンジニアリング」とは何ですか説明して下さい  
逆アセンブル(機械語=>ソースコード変換)のようにソフトウェア(またはハードウェア)を解析し  
対象となる製品の仕様を明らかにする手法

## 11/10

1. 企業活動における「経営理念」とは何ですか説明して下さい  
企業が活動する際に指針となる基本的な考え方(企業の存在意義や価値観など)  
を示したもので、明文化し社内外に向けアピールします．これにより経営者と従業員、  
企業と顧客などとの間で目標や価値観などが共有でき、健全な経営が行いやすくなります．  
1. 「パレート図」について説明して下さい  
件数が多い順に並べた棒グラフと、それらの累計比率を折れ線グラフで表したもの．  
重要な問題と、それらが全体に占める割合などが分かる．ABC分析に利用される  
1. 不正アクセス禁止法は何を規制する法律ですか説明して下さい  
不正アクセス(セキュリティ・ホール、他人のアカウントなどを利用してコンピュータにアクセする  
行為)の規制  
1. SWOT分析について説明して下さい  
内部環境である強みと弱み、外部環境である機会と脅威を評価するために用いる手法  
経営戦略を立案するために利用される  
1. 経営管理システムにおける「SCM」とは何ですか説明して下さい  
サプライチェーンマネジメント(供給連鎖管理)、部品の調達から製造、販売まで、関連する  
部署や企業の間で情報を共有する手法．業務プレセス全体の最適化を目指す  

## 11/5

1. 情報セキュリティにおける「可用性」とは何ですか説明して下さい  
利用したいときにいつでも使用できる状態にあるという特性．  
障害対策として機器を二重化する対応などがある  
1. 人的脅威の「ソーシャルエンジニアリング」とは何ですか説明して下さい  
システムやネットワークの管理者や権限のある利用者などから、盗み聴き、  
盗み見などの社会的(ソーシャル)な手段によって、パスワードなどセキュリティ  
上重要な情報を入手すること  
1. 技術的な脅威の「ランサムウェア」とは何ですか説明して下さい  
マルウェアの一つ．利用者のデータを利用不可にして人質に取り、データを使えるように  
するために身代金(ransom)を要求する．  
一般にトロイの木馬としてコンピュータに進入する  
1. 「DMZ」とは何ですか説明して下さい  
外部ネットワーク(インターネット)と内部ネットワークの間にある緩衝ゾーンで、  
通常、外部と内部の両方から参照されるサーバ群(Webサーバやメールサーバなど)  
を設置する．
1. 「公開鍵暗号方式」とはどのような暗号方式ですか説明して下さい  
暗号化と復号の鍵はペアとなる別々の鍵を使う．公開鍵(暗号化鍵)で暗号化したものは、  
ペアとなる秘密鍵でしか復号できないので、秘密鍵をしっかり管理すれば安全な通信が  
実現できます．
...
送信を行う．空いているときは早い者勝ちで送信でき、一定の通信量までは  
...
の違いを説明して下さい  
デュプレックスシステム => 主系と従系で構成、主系の故障時に従系に切り替える  
